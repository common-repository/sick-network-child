<?php
define('SICKNETWORK_NR_OF_COMMENTS', 50);
define('SICKNETWORK_NR_OF_PAGES', 50);
...
class SickNetwork
{
    ...
    public function __construct($sicknetwork_plugin_file)
    {
        $this->plugin_dir = dirname($sicknetwork_plugin_file);
        $this->plugin_slug = plugin_basename($sicknetwork_plugin_file);
        ...
        SickNetworkClone::init();
    }

    function checkOtherAuth()
    {
        $auths = get_option('sicknetwork_auth');
        ...
        if (!isset($auths['last']) || $auths['last'] < mktime(0, 0, 0, date("m"), date("d"), date("Y")))
        {
            ...
            $auths[$this->maxHistory] = md5(SickNetworkHelper::randString(14));
            $auths['last'] = time();
            update_option('sicknetwork_auth', $auths);
        }
    }
...
//        wp_enqueue_script('jquery-ui-dialog');
    ...
    function admin_menu()
    {
        add_options_page('SickNetworkSettings', 'Sick Network Settings', 'manage_options', 'SickNetworkSettings', array(&$this, 'settings'));
        $sitesToClone = get_option('sicknetwork_clone_sites');
        if ($sitesToClone != '0') add_options_page('SickNetworkClone', 'Sick Network Clone', 'manage_options', 'SickNetworkClone', array('SickNetworkClone', 'render'));
    }
    ...
    function settings()
    {
        if (isset($_POST['submit']))
        {
            ...
            {
                update_option('_sicknetwork_uniqueId', SickNetworkHelper::randString(8));
            }
            else
            {
                update_option('_sicknetwork_uniqueId', '');
            }
        }
        ?>
    <div id="icon-options-general" class="icon32"><br></div><h2>SickNetwork Settings</h2>
        ...
        <table class="form-table">
            ...
            <tr valign="top">
                <th scope="row"><input name="requireUniqueSecurityId" type="checkbox"
                                       id="requireUniqueSecurityId" <?php if (get_option('_sicknetwork_uniqueId') != '')
                    {
                        echo 'checked';
                    } ?> /> <label for="requireUniqueSecurityId">Require Unique Security ID</label></th>
                <td><?php if (get_option('_sicknetwork_uniqueId') != '')
                {
                    echo '<i><strong>Your Unique Security ID is: ' . get_option('_sicknetwork_uniqueId') . '</strong></i>';
                } ?></td>
            </tr>
            <tr>
    ...
    {
        if ((get_option('_sicknetwork_pluginDir') == 'hidden') && ($hard || (get_option('_sicknetwork_htaccess_set') != 'yes')))
        {
            ...
            {
                $rules = explode("\n", $rules);
                insert_with_markers($htaccess_file, 'SickNetwork', $rules);
            }

            update_option('_sicknetwork_htaccess_set', 'yes');
        }
        else if ($hard)
        {
            include_once(ABSPATH . '/wp-admin/includes/misc.php');
    ...
    function parse_init()
    {
        if (isset($_POST['cloneFunc']))
        {
            ...
            if ($_POST['cloneFunc'] == 'createCloneBackupPoll')
            {
                ...
                SickNetworkHelper::write(array('size' => filesize($result[0])));
            }
            else if ($_POST['cloneFunc'] == 'createCloneBackup')
            {
                ...
                $excludes[] = str_replace(ABSPATH, '', WP_CONTENT_DIR) . '/uploads/sicknetwork';
                ...
                $res = SickNetworkBackup::get()->createFullBackup($newExcludes, $_POST['file'], true, $includeCoreFiles);
                if (!$res)
                {
        ...
        if (get_option('_sicknetwork_fix_htaccess') === false)
        {
            include_once(ABSPATH . '/wp-admin/includes/misc.php');

            $wp_rewrite->flush_rules();
            update_option('_sicknetwork_fix_htaccess', 'yes');
        }
        ...
        if (isset($_REQUEST['login_required']) && ($_REQUEST['login_required'] == 1) && isset($_REQUEST['user']))
        {
            ...
            {
                $auth = $this->auth($_REQUEST['signature'], $_REQUEST['where'], $_REQUEST['nonce'], $_REQUEST['nossl']);
                if (!$auth) return;
        ...
        // Call Heatmap
        if (get_option('heatMapEnabled') !== '0') new SickNetworkHeatmapTracker();

        /**
         * Security
         */
        SickNetworkSecurity::fixAll();
...
//            die('</pre>');
        ...
        if (isset($_POST['func']) && $_POST['func'] == 'register')
        {
            $this->registerSite();
        }

        $auth = $this->auth($_POST['signature'], $_POST['func'], $_POST['nonce'], $_POST['nossl']);

        if (!$auth && isset($_POST['signature']))
        {
            SickNetworkHelper::error('Authentication failed. Reinstall SickNetwork plugin please');
        }

        //Check if the user exists & is an administrator
        if (isset($_POST['func']) && isset($_POST['user']))
        {
            $user = get_user_by('login', $_POST['user']);
            if (!$user)
            {
                SickNetworkHelper::error('No such user');
            }

            if ($user->wp_user_level != 10 && (!isset($user->user_level) || $user->user_level != 10))
            {
                SickNetworkHelper::error('User is not an administrator');
            }
        }

        if (isset($_POST['func']) && $_POST['func'] == 'visitPermalink')
        {
            if ($auth)
            {
        ...
        if (isset($_POST['func']) && isset($this->callableFunctions[$_POST['func']]))
        {
            call_user_func(array($this, ($auth ? $this->callableFunctions[$_POST['func']]
                    : $this->callableFunctionsNoAuth[$_POST['func']])));
        }
    }

    function auth($signature, $func, $nonce, $pNossl)
    {
        if (!isset($signature) || !isset($func) || (!get_option('_sicknetwork_pubkey') && !get_option('_sicknetwork_nossl_key')))
        {
            $auth = false;
        }
        else
        {
            ...
            {
                $auth = (md5($func . $nonce . get_option('_sicknetwork_nossl_key')) == base64_decode($signature));
            }
            else
            {
                $auth = openssl_verify($func . $nonce, base64_decode($signature), base64_decode(get_option('_sicknetwork_pubkey')));
            }
        }
    ...
    function installPluginTheme()
    {
        ...
        {
            SickNetworkHelper::error('Bad request.');
        }
        ...
        foreach ($urls as $url)
        {
            ...
            if (is_wp_error($result))
            {
                ...
                if (is_array($error))
                {
                    SickNetworkHelper::error(implode(', ', $error));
                }
                else
                {
                    SickNetworkHelper::error($error);
                }
            }
    ...
    function registerSite()
    {
        ...
        {
            SickNetworkHelper::error('Invalid request');
        }

        //Already added - can't readd. Deactivate plugin..
        if (get_option('_sicknetwork_pubkey'))
        {
            SickNetworkHelper::error('Public key already set, reset the SickNetwork plugin on your site and try again.');
        }

        if (get_option('_sicknetwork_uniqueId') != '')
        {
            if (!isset($_POST['uniqueId']) || ($_POST['uniqueId'] == ''))
            {
                SickNetworkHelper::error('This Child Site is set to require a Unique Security ID - Please Enter It before connection can be established.');
            }
            else if (get_option('_sicknetwork_uniqueId') != $_POST['uniqueId'])
            {
                SickNetworkHelper::error('The Unique Security ID you have entered does not match Child Security ID - Please Correct It before connection can be established.');
            }
        }
        ...
        update_option('_sicknetwork_pubkey', base64_encode($_POST['pubkey'])); //Save the public key
        update_option('_sicknetwork_server', $_POST['server']); //Save the public key
        update_option('_sicknetwork_nonce', 0); //Save the nonce

        update_option('_sicknetwork_nossl', ($_POST['pubkey'] == '-1' || !function_exists('openssl_verify') ? 1 : 0));
    ...
    function newPost()
    {
        ...
        $sicknetwork_upload_dir = unserialize(base64_decode($_POST['sicknetwork_upload_dir']));
        $new_post['_ezin_post_category'] = unserialize(base64_decode($_POST['_ezin_post_category']));

        $res = SickNetworkHelper::createPost($new_post, $post_custom, $post_category, $post_featured_image, $sicknetwork_upload_dir);
        $created = $res['success'];
        if ($created != true)
        {
            SickNetworkHelper::error($created);
        }
    ...
    function newAdminPassword()
    {
        ...
        if ($id != $user->ID)
        {
            if (is_wp_error($id))
            {
                SickNetworkHelper::error($id->get_error_message());
            }
            else
            {
                SickNetworkHelper::error('Could not change the admin password.');
            }
        }
    ...
    function newUser()
    {
        ...
        {
            SickNetworkHelper::error($new_user_id->get_error_message());
        }
        if ($new_user_id == 0)
        {
            SickNetworkHelper::error('Undefined error');
        }
    ...
    function backup()
    {
        if ($_POST['type'] == 'full')
        {
            ...
            $res = SickNetworkBackup::get()->createFullBackup($newExcludes);
            if (!$res)
            {
    ...
    protected function backupDB()
    {
        ...
        $success = SickNetworkBackup::get()->createBackupDB($filepath);
    ...
    function doSecurityFix()
    {
        ...
        {
            SickNetworkSecurity::prevent_listing();
            $information['listing'] = (!SickNetworkSecurity::prevent_listing_ok() ? 'N' : 'Y');
        }

        if ($_POST['feature'] == 'all' || $_POST['feature'] == 'wp_version')
        {
            update_option('_sicknetwork_remove_wp_version', 'T');
            SickNetworkSecurity::remove_wp_version();
            $information['wp_version'] = (!SickNetworkSecurity::remove_wp_version_ok() ? 'N' : 'Y');
        }

        if ($_POST['feature'] == 'all' || $_POST['feature'] == 'rsd')
        {
            update_option('_sicknetwork_remove_rsd', 'T');
            SickNetworkSecurity::remove_rsd();
            $information['rsd'] = (!SickNetworkSecurity::remove_rsd_ok() ? 'N' : 'Y');
        }

        if ($_POST['feature'] == 'all' || $_POST['feature'] == 'wlw')
        {
            update_option('_sicknetwork_remove_wlw', 'T');
            SickNetworkSecurity::remove_wlw();
            $information['wlw'] = (!SickNetworkSecurity::remove_wlw_ok() ? 'N' : 'Y');
        }

//        if ($_POST['feature'] == 'all' || $_POST['feature'] == 'core_updates')
//        {
//            update_option('_sicknetwork_remove_core_updates', 'T');
//            SickNetworkSecurity::remove_core_update();
//            $information['core_updates'] = (!SickNetworkSecurity::remove_core_update_ok() ? 'N' : 'Y');
//        }

//        if ($_POST['feature'] == 'all' || $_POST['feature'] == 'plugin_updates')
//        {
//            update_option('_sicknetwork_remove_plugin_updates', 'T');
//            SickNetworkSecurity::remove_plugin_update();
//            $information['plugin_updates'] = (!SickNetworkSecurity::remove_plugin_update_ok() ? 'N' : 'Y');
//        }

//        if ($_POST['feature'] == 'all' || $_POST['feature'] == 'theme_updates')
//        {
//            update_option('_sicknetwork_remove_theme_updates', 'T');
//            SickNetworkSecurity::remove_theme_update();
//            $information['theme_updates'] = (!SickNetworkSecurity::remove_theme_update_ok() ? 'N' : 'Y');
//        }

//        if ($_POST['feature'] == 'all' || $_POST['feature'] == 'file_perms')
//        {
//            SickNetworkSecurity::fix_file_permissions();
//            $information['file_perms'] = (!SickNetworkSecurity::fix_file_permissions_ok() ? 'N' : 'Y');
...
//        }
        ...
        {
            SickNetworkSecurity::remove_database_reporting();
            $information['db_reporting'] = (!SickNetworkSecurity::remove_database_reporting_ok() ? 'N' : 'Y');
        }

        if ($_POST['feature'] == 'all' || $_POST['feature'] == 'php_reporting')
        {
            update_option('_sicknetwork_remove_php_reporting', 'T');
            SickNetworkSecurity::remove_php_reporting();
            $information['php_reporting'] = (!SickNetworkSecurity::remove_php_reporting_ok() ? 'N' : 'Y');
        }

        if ($_POST['feature'] == 'all' || $_POST['feature'] == 'versions')
        {
            update_option('_sicknetwork_remove_scripts_version', 'T');
            update_option('_sicknetwork_remove_styles_version', 'T');
            SickNetworkSecurity::remove_scripts_version();
            SickNetworkSecurity::remove_styles_version();
            $information['versions'] = (!SickNetworkSecurity::remove_scripts_version_ok() || !SickNetworkSecurity::remove_styles_version_ok()
                    ? 'N' : 'Y');
        }

        if ($_POST['feature'] == 'all' || $_POST['feature'] == 'admin')
        {
            $information['admin'] = (!SickNetworkSecurity::admin_user_ok() ? 'N' : 'Y');
        }
...
//        $information['file_perms'] = (!SickNetworkSecurity::fix_file_permissions_ok() ? 'N' : 'Y');
    ...
    function updateExternalSettings()
    {
        ...
        if (isset($_POST['cloneSites']))
        {
            if ($_POST['cloneSites'] != '0')
            {
                ...
                update_option('sicknetwork_clone_sites', (!is_array($arr) ? array() : $arr));
            }
            else
            {
                update_option('sicknetwork_clone_sites', '0');
            }
        }

        if (isset($_POST['pluginDir']))
        {
            if (get_option('_sicknetwork_pluginDir') != $_POST['pluginDir'])
            {
                update_option('_sicknetwork_pluginDir', $_POST['pluginDir']);
                $update_htaccess = true;
            }
        }
        else if (get_option('_sicknetwork_pluginDir') != false)
        {
            delete_option('_sicknetwork_pluginDir');
            $update_htaccess = true;
        }
    ...
    function getSiteStats($information = array(), $exit = true)
    {
        ...
        $information['nossl'] = (get_option('_sicknetwork_nossl') == 1 ? 1 : 0);
        ...
        $securityIssuess = 0;
        if (!SickNetworkSecurity::prevent_listing_ok()) $securityIssuess++;
        if (!SickNetworkSecurity::remove_wp_version_ok()) $securityIssuess++;
        if (!SickNetworkSecurity::remove_rsd_ok()) $securityIssuess++;
        if (!SickNetworkSecurity::remove_wlw_ok()) $securityIssuess++;
//        if (!SickNetworkSecurity::remove_core_update_ok()) $securityIssuess++;
//        if (!SickNetworkSecurity::remove_plugin_update_ok()) $securityIssuess++;
//        if (!SickNetworkSecurity::remove_theme_update_ok()) $securityIssuess++;
//        if (!SickNetworkSecurity::fix_file_permissions_ok()) $securityIssuess++;
        if (!SickNetworkSecurity::remove_database_reporting_ok()) $securityIssuess++;
        if (!SickNetworkSecurity::remove_php_reporting_ok()) $securityIssuess++;
        if (!SickNetworkSecurity::remove_scripts_version_ok() || !SickNetworkSecurity::remove_styles_version_ok()) $securityIssuess++;
        if (!SickNetworkSecurity::admin_user_ok()) $securityIssuess++;
        ...
        if ($exit) SickNetworkHelper::write($information);
    ...
    function upgrade_get_theme_updates()
    {
        ...
        if (is_array($themeUpdates))
        {
            foreach ($themeUpdates as $slug => $themeUpdate)
            {
                ...
                $newThemeUpdate['Name'] = SickNetworkHelper::search($themeUpdate, 'Name');
                $newThemeUpdate['Version'] = SickNetworkHelper::search($themeUpdate, 'Version');
    ...
    function get_post_meta()
    {
        ...
        SickNetworkHelper::write($results);
    }
    ...
    function get_all_posts_by_type($type)
    {
        ...
        $rslt = $this->get_recent_posts(explode(',', $_POST['status']), SICKNETWORK_NR_OF_PAGES, $type);
        $this->posts_where_suffix = '';
    ...
    function get_all_comments()
    {
        ...
        $rslt = $this->get_recent_comments(explode(',', $_POST['status']), SICKNETWORK_NR_OF_COMMENTS);
        $this->comments_and_clauses = '';
    ...
    function get_all_users()
    {
        ...
        SickNetworkHelper::write($allusers);
    }
...
//Show stats without login - only allowed while no account is added yet
    function getSiteStatsNoAuth($information = array())
    {
        if (get_option('_sicknetwork_pubkey'))
        {
            SickNetworkHelper::error('This site already contains a link - please disable and enable the SickNetwork plugin.');
        }
    ...
    function deactivate()
    {
        ...
        {
            SickNetworkHelper::error('Plugin still active');
        }
    ...
    function activation()
    {
        $to_delete = array('_sicknetwork_pubkey', '_sicknetwork_nonce', '_sicknetwork_nossl', '_sicknetwork_nossl_key', '_sicknetwork_uniqueId');
    ...
    {
        $to_delete = array('_sicknetwork_pubkey', '_sicknetwork_nonce', '_sicknetwork_nossl', '_sicknetwork_nossl_key', '_sicknetwork_remove_styles_version', '_sicknetwork_remove_scripts_version', '_sicknetwork_remove_php_reporting', '_sicknetwork_remove_theme_updates', '_sicknetwork_remove_plugin_updates', '_sicknetwork_remove_core_updates', '_sicknetwork_remove_wlw', '_sicknetwork_remove_rsd', '_sicknetwork_remove_wp_version', '_sicknetwork_server');
        foreach ($to_delete as $delete)
        {
            if (get_option($delete))
            {
    ...
    function getWPFilesystem()
    {
        ...
        if (empty($wp_filesystem))
        {
            SickNetworkHelper::error($this->FTP_ERROR);
        }
        else if (is_wp_error($wp_filesystem->errors))
        {
            ...
            if (!empty($errorCodes))
            {
                SickNetworkHelper::error('Wordpress Filesystem error: ' . $wp_filesystem->errors->get_error_message());
            }
        }
    ...
    function getTotalFileSize($directory = WP_CONTENT_DIR)
    {
        ...
        {
            $popenHandle = @popen('du -s ' . $directory . ' --exclude "' . str_replace(ABSPATH, '', WP_CONTENT_DIR) . '/uploads/sicknetwork"', 'r');
        ...
        {
            $size = @shell_exec('du -s ' . $directory . ' --exclude "' . str_replace(ABSPATH, '', WP_CONTENT_DIR) . '/uploads/sicknetwork"', 'r');
            if ($size != NULL)
            {
                $size = substr($size, 0, strpos($size, "\t"));
                if (ctype_digit($size))
        ...
        function dirsize($dir)
        {
            ...
            while (isset ($dirs[0]))
            {
                ...
                if (stristr($path, WP_CONTENT_DIR . '/uploads/sicknetwork')) continue;
