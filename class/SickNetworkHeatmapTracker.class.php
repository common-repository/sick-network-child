...
class SickNetworkHeatmapTracker
{
	...
	{
		self::$instance = $this;
        $this->server = get_option('_sicknetwork_server');
	...
	public function saveClickCallback()
	{
		...
		$storeData = get_option('_sicknetwork_click_data');
		...
		update_option('_sicknetwork_click_data', $storeData);
	...
	public function sendClick()
	{
		...
		$clickData = get_option('_sicknetwork_click_data');
		$key = get_option('_sicknetwork_pubkey');
		...
		if ( is_array($request) && intval($request['body']) > 0 )
			delete_option('_sicknetwork_click_data');
	}
